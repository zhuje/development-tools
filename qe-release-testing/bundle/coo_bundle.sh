...
# Apply Namespace (must be flush left)
echo "Applying Namespace openshift-cluster-observability-operator..."
oc apply -f - <<EOF
apiVersion: v1
kind: Namespace
metadata:
...
if [ "$choice_index" -eq 1 ]; then
  ...
  operator-sdk run bundle "${bundle}" --install-mode AllNamespaces
else
