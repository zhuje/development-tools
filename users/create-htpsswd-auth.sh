...
# Confirm signed in as kubeadmin (needed to grant permissions to user)
## oc whoami > if not kubeadmin > exit

# Using default user
echo "Using default test user"
echo "username: ${GREEN} user ${ENDCOLOR}"
...
# Sleep -- wait for user account to be created
echo " ${GREEN}** Sleeping for 60s to await user account creation ** ${ENDCOLOR}"
sleep 60
echo "\n"
...
# Apply permissions to user - you must be logged in a cluster administratior (e.g. kubeadmin)
echo "\n** Granting permission for  \n user: ${GREEN} ${DEFAULT_USER}  ${ENDCOLOR}   \n namepsace: ${GREEN} ${NAMESPACE} ${ENDCOLOR}\n "

echo "${GREEN}Granting policy add-role-to-user view ${ENDCOLOR}  "
oc -n ${NAMESPACE} policy add-role-to-user view ${DEFAULT_USER}

echo "${GREEN}Granting policy add-role-to-user cluster-logging-application-view ${ENDCOLOR}  "
oc -n ${NAMESPACE} policy add-role-to-user cluster-logging-application-view ${DEFAULT_USER}

echo "${GREEN}Granting policy add-role-to-user monitoring-rules-edit ${ENDCOLOR}  "
oc -n ${NAMESPACE} policy add-role-to-user monitoring-rules-edit ${DEFAULT_USER}

echo "${GREEN}Granting policy add-role-to-user cluster-monitoring-view ${ENDCOLOR} "
oc -n ${NAMESPACE} policy add-role-to-user cluster-monitoring-view ${DEFAULT_USER}
